...
export class LLMChunker {
    ...
    private createWorker(chunks: string[]): Promise<number[]> {
        return new Promise((resolve, reject) => {
            const worker = new Worker(path.resolve(__dirname, "workers.ts"), {
                workerData: { chunks },
            });
    ...
    async chunkText(text: string): Promise<string[]> {
        ...
        try {
            if (!text || text.length < this.minChunkSize) {
                console.log(
                    "[LLMChunker] Text too short, returning as single chunk"
                );
                return text ? [text] : [];
            }
            ...
            console.log(
                `[LLMChunker] Created ${initialChunks.length} initial chunks`
            );

            if (initialChunks.length <= 1) {
                console.log(
                    "[LLMChunker] Single chunk, no further processing needed"
                );
            ...
            for (
                ...
                for (
                    let j = 0;
                    j < this.maxWorkers &&
                    i + j * batchSize < initialChunks.length;
                    j++
                ) {
                ...
                console.log(
                    `[LLMChunker] Processing batch of ${batchChunks.length} windows`
                );
            ...
            console.log(
                `[LLMChunker] Chunking complete. Final chunks: ${finalChunks.length}`
            );
            return finalChunks;
        } catch (error) {
